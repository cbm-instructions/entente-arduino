...
const char introMessage[]      = {"<SNAKE> PRESS TO START TO PLAY"};
const char gameOverMessage[]  = {"GAME OVER SCORE"};
...
// if position error appears (Hardware error)
boolean isPositionError = false;
int position_error_x = 1;
int position_error_y = 1;
...
void setup() {
  ...
  if ( debug_mode || debug_input ) {
    Serial.begin (1200); // bps rates: 300, 600, 1200, 2400, 4800, 9600, 14400, 19200, 28800, 38400, 57600, or 115200
  }
...
void print_all() {
  ...
  if ( ( current_tick_time - last_print_time ) >= print_tick_intervall ) {
    ...
    if(isPositionError) {
      tmp_color = matrix.Color(111, 111, 111);
      matrix.drawPixel(position_error_x, position_error_y, tmp_color); 
    }
...
void check_saved_input() {
  if ( saveLastInputs[0] == WEST && saveLastInputs[1] == EAST && saveLastInputs[2] == EAST && saveLastInputs[3] == EAST && saveLastInputs[4] == EAST) {
    isRainbowMode = true;
  }
...
void set_position() {
  ...
  //if(position_x >= 0 && position_x < NUM_LEDS_X && position_y >= 0 && position_y < NUM_LEDS_Y) {
...
void printSide(Side side) {
  if (side == NORTH) {
    ...
    Serial.println("NORTH_EAST");
  } else if (side == SOUTH_EAST) {
    Serial.println("SOUTH_EAST");
  } else if (side == SOUTH_WEST) {
    Serial.println("SOUTH_WEST");
  } else if (side == NORTH_WEST) {
    Serial.println("NORTH_WEST");
  }
}
...
void shift_snake(int head_position_x, int head_position_y) {
  ...
  // snake_length is max in size because its not dynamic -> use of current fill-size
...
void set_direction() {
  if ( joystick_input == WEST && current_direction != EAST) {
    ...
    if (isRainbowMode || !isRainbowMode && saveLastInputs[1] != NORTH_WEST) {
    ...
    if (isRainbowMode || !isRainbowMode && saveLastInputs[1] != NORTH_EAST) {
      if (current_direction  == WEST || current_direction  == EAST) {
        last_direction = current_direction;
        current_direction = NORTH;
      } else if (current_direction == NORTH || current_direction == SOUTH) {
    ...
    if (isRainbowMode || !isRainbowMode && saveLastInputs[1] != SOUTH_EAST) {
      if (current_direction  == WEST ||  current_direction == EAST) {
        last_direction = current_direction;
    ...
    if (isRainbowMode || !isRainbowMode && saveLastInputs[1] != SOUTH_WEST) {
      if (current_direction  == WEST ||  current_direction == EAST) {
        last_direction = current_direction;
        current_direction = SOUTH;
      } else if (current_direction == NORTH || current_direction == SOUTH) {
...
//This Method checks if the direction is valid. It is valid if the x and y coordinate is over an min value (=> min shift position)
boolean isValidDirection(Side direction) {
  if (isPositionError) {
    return true;
  }
  ...
  if (direction == NORTH) {
    if (y > JOYSTICK_MIN_POSITION) return true;
  } else if (direction == EAST) {
    if (x > JOYSTICK_MIN_POSITION) return true;
  } else if (direction == SOUTH) {
    if (y < -JOYSTICK_MIN_POSITION) return true;
  } else if (direction == WEST) {
    if (x < -JOYSTICK_MIN_POSITION) return true;
  } else if (direction == NORTH_WEST) {
    if (x < -JOYSTICK_MIN_POSITION && y > JOYSTICK_MIN_POSITION) return true;
...
void get_input() {
  ...
  if (stick.isRightUp()) {
    if (isValidDirection(NORTH_EAST)) {
      if ( debug_input  ) {
  ...
  if (stick.isRightDown()) {
    if (isValidDirection(SOUTH_EAST)) {
      if ( debug_input  ) {
  ...
  if (stick.isLeftDown()) {
    if (isValidDirection(SOUTH_WEST)) {
      if ( debug_input  ) {
  ...
  if (stick.isLeftUp()) {
    if (isValidDirection(NORTH_WEST)) {
      if (debug_input  ) {
  ...
  //check if position x y error with joystick (hardware error)
  if(stick.isCenter()) {
    isPositionError = checkIfPositionError();
  }
}

//checks if position x y error with joystick (hardware error) exists
boolean checkIfPositionError() {
  int x = stick.getX();
  int y = stick.getY();
  if (x < -5 && x > 5 || y < -5 && y > 5) {
    return true;
